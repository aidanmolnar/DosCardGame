use dos_shared::{
    dos_game::DosGame,
    messages::game::{CallDosInfo, GameAction},
};
...
pub fn call_dos_graceperiod_system(mut network_manager: GameNetworkManager, time: Res<Time>) {
    if let Some(graceperiod) = &mut network_manager.game.call_dos.as_mut().unwrap().graceperiod {
        if graceperiod.finished() {
            ...
            network_manager
                .game
                .punish_missed_dos(network_manager.game.call_dos.as_ref().unwrap().player);
            network_manager.send_to_all(GameAction::CallDos(Some(CallDosInfo {
                player: network_manager.game.call_dos.as_ref().unwrap().player,
                caller: network_manager
                    .game
                    .call_dos
                    .as_ref()
                    .unwrap()
                    .caller
                    .expect("Caller must be set when timer is set"),
            })));

use dos_shared::{
    ...
    messages::lobby::{GameSnapshot, TableSnapshot},
...
impl ServerGame<'_, '_> {
    ...
    fn record_card_value(&mut self, from: &Location, to: &Location, card: Card) {
        for player in 0..self.game_info.num_players() {
            if !self.is_visible(from, player) && self.is_visible(to, player) {
...
impl CardTransfer<Card, ServerTable> for ServerGame<'_, '_> {
    fn get_table(&self, location: &Location) -> &ServerTable {
        let entity = *self
            .map
            .0
            .get(location)
        ...
        self.tables
            .get(entity)
            .expect("Table does not exist on table entity")
    }

    fn get_table_mut(&mut self, location: &Location) -> &mut ServerTable {
        let entity = *self
            .map
            .0
            .get(location)
...
impl DosGame<Card, ServerTable> for ServerGame<'_, '_> {
    ...
    fn someone_has_two_cards(&mut self, player: usize) {
        println!("player with id {player} has two cards!");

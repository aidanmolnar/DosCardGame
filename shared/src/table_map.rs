...
impl Plugin for TableConstructionPlugin {
    fn build(&self, app: &mut App) {
        app.add_loopless_state(TableConstructionState::NotStarted)
            // State chain for building tables
            .add_exit_system(GameState::MainMenu, |mut commands: Commands| {
                commands.insert_resource(NextState(TableConstructionState::TableMapCreation));
            })
            .add_enter_system(
                TableConstructionState::TableMapCreation,
                |mut commands: Commands| {
                    commands.insert_resource(NextState(TableConstructionState::TableCreation));
                },
            )
            .add_enter_system(
                TableConstructionState::TableCreation,
                |mut commands: Commands| {
                    commands.insert_resource(NextState(TableConstructionState::Completed));
                },
            )
...
pub fn build_table_map(mut commands: Commands, num_players: usize) {
    let mut map = TableMap::default();

    map.0.insert(Location::Deck, commands.spawn_empty().id());

    map.0
        .insert(Location::DiscardPile, commands.spawn_empty().id());

    map.0.insert(Location::Staging, commands.spawn_empty().id());

    spawn_player_hand_tables(&mut map, &mut commands, num_players);

    commands.insert_resource(map);
}

fn spawn_player_hand_tables(map: &mut TableMap, commands: &mut Commands, num_players: usize) {
    for player_id in 0..num_players {
        map.0
            .insert(Location::Hand { player_id }, commands.spawn_empty().id());
    }
}

fn remove_tables(table_map: Res<TableMap>, mut commands: Commands) {

...
use super::{call_dos::CallDos, client_game::ClientGame};
...
pub fn game_network_system(mut network_manager: GameNetworkManager) {
    // Read all game messages from server
    while let Some(message) = network_manager
        .renet_client
        .receive_message(GAME_CHANNEL_ID)
    {
        let update =
            bincode::deserialize::<FromServer>(&message).expect("Failed to deserialize message!");

...
fn getPoint(id: vec2<f32>, offset: vec2<f32>) -> vec2<f32> {
    return offset + sin(n22(id + offset) * (toffset + globals.time) * tscale * 1.0) * 0.4;
}

fn layer(uv: vec2<f32>) -> f32 {
    var m = 0.0;
    let t = (toffset + globals.time) * tscale * 2.0;
...
fn fragment(in: FragmentInput) -> @location(0) vec4<f32> {
    ...
    var c = sin((toffset + globals.time) * tscale * 2.0 * vec3<f32>(.934, .324, .768)) * 0.3 + 0.7;
